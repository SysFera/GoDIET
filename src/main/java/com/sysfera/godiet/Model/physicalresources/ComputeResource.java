...
public class ComputeResource extends Resources {
	...
	public ComputeResource(String name, ComputeCollection collection,Domain domain) {
		super(name,domain);
	...
	/*
	 * public Exception throwJobsExistException() throws Exception { Exception x
	 * = new Exception("Machine " + outerThis.getName() + " has " +
	 * outerThis.jobCount + " active jobs.  Can not" + " change the config.");
	 * throw (x); }
	 */
	...
	/**
	 * If user specified constraints on ports to be used as end points, the next
	 * unallocated port is allocated and returned. If the user did not specify
	 * constraints or all ports have been allocated, -1 is returned.
	 */
	public int allocateAllowedPort() {
		...
		if ((this.endAllowedPorts > 0)
				&& ((this.begAllowedPorts + this.allocatedPorts) >= this.endAllowedPorts)) {
			System.err.println("Port allocation on " + this.getName()
					+ " failed.  All ports [" + this.begAllowedPorts + ","
					+ this.endAllowedPorts + "] already allocated.");
			return -1;
		}
