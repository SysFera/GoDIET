...
\godiet needs to have the description of infrastructure on which \diet will be running. Cette description doit être fait dans un fichier au format XML. Il listera
tous les éléments physiques indispensables au fonctionnement et au déploiement de \diet.

Les principaux éléments sont:
\begin{itemize}
\item \textbf{Domain}: définit un ensemble d'élements (node ou gateway) qui peuvent communiquer directement en utilisant le protocole d'échange de \diet (i.e CORBA). Dans le cas où des machines sont séparer par un dispositif de filtrage (mur en feu ou autres), il faut les décrire dans deux domaines différents.
\item \textbf{Storage}: définit un espace de stockage distant.
\item \textbf{Node}: définit une machine physique sur laquelle sera executé les logiciels, typiquement les \seds et les agents.
\item \textbf{Gateway}: définit une passerelle réseau. Elle permette de créer les interconnections entre les différents domaines.
...
Créer votre fichier \verb+${HOME}/.godiet/configuration.xml+
Charger votre description d'infrastructure\ref{GODIETInfrastructureDescription}
Charger votre description de plate-forme \diet\ref{GODIETPlatformDesc}
Démarrer et arrêter vos composants\ref{GODIETCommandStartStop}
...
\begin{itemize}
\item \textbf{software} SoftwareName: Start one software given his name.
\item \textbf{services}: Start all services
\item \textbf{agents}:  Start all agents
\item \textbf{seds}: Start all seds
\item \textbf{all}: Start all softwares register
\end{itemize}
\verb+stop+ command takes the same parameters.
